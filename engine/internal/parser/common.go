...
// layout has not changed between F1 23 and F1 26.
func parseLapData(data []byte, numCars int) (f1.LapPayload, bool) {
	required := f1.HeaderSize + numCars*lapDataSize
	if len(data) < required {
		return f1.LapPayload{}, false
	}
	var out f1.LapPayload
	for i := 0; i < numCars; i++ {
		base := f1.HeaderSize + i*lapDataSize
