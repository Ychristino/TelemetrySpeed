...
// Frame holds the assembled data for one frameIdentifier, for the player's
// car only.
type Frame struct {
	SessionID uuid.UUID
	FrameID   uint32
...
// ToTelemetryRow converts a finalized Frame into a database row for the
// player's car. Returns ok=false if the player is inactive/invalid (e.g.
// still in the garage), in which case nothing should be written.
// Columns must match telemetryCols in writer.go exactly.
func (fr *Frame) ToTelemetryRow() (database.TelemetryRow, bool) {
	c := fr.Car
	if c.ResultStatus <= f1.ResultStatusInactive {
		return database.TelemetryRow{}, false
	}
	row := database.TelemetryRow{
		SessionID: fr.SessionID,
		CarIndex:  fr.CarIndex,
